...
(provide Var Var? make-Var App App? make-App Abs Abs? make-Abs DBvar DBvar? make-DBvar DBapp DBapp? make-DBapp DBabs DBabs? make-DBabs parse lambda-interp subst)
...
;;struct definitions for a debruijn ast. You must use these structs to define your debruijn expressions.
(define-struct DBvar (id) #:transparent)
(define-struct DBapp (fst snd) #:transparent)
(define-struct DBabs (bdy) #:transparent)
