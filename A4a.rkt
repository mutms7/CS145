...
(define (fthelp t h) ;; 
  (cond
    [(= h 0) t]
    [(empty? t) (fthelp (make-node t t 1) (sub1 h))]
    [else (fthelp (make-node t t (+ 1 (* 2 (node-sz t)))) (sub1 h))]
  )
)
...
(define (tree-grow-min n)
  (cond
    [(empty? n) (make-node empty empty 1)]
    [(> (tree-size (node-left n)) (tree-size (node-right n)))
       (make-node
        (node-left n)
        (tree-grow-min
         (node-right n)) (add1 (node-sz n)))
       ]
    [else
     (make-node
        (tree-grow-min
