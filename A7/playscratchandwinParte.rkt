...
;; For use by students in CS145 Fall 2016, 2017, 2018, 2024
...
(define (subst-L t p) (build-node (first (scratch  (node-key t))) p (node-right t)))

(define (subst-R t p) (build-node (first (scratch  (node-key t))) (node-left t) p))
...
(define (insertavl t d)
  (cond
    ...
    [(= (first (scratch d)) (first (scratch (node-key t)))) t]
    [(< (first (scratch d)) (first (scratch (node-key t)))) (rebalance (subst-L t (insertavl (node-left t) d)))]
    [else (rebalance (subst-R t (insertavl (node-right t) d)))]))

(define (get_rightmost t)
  (cond
    [(empty? (node-right t)) (first (scratch  (node-key t)))]
...
(define (deleteavl t d)
  (cond
    [(empty? t) t]
    [(< d (first (scratch  (node-key t)))) (rebalance (subst-L t (deleteavl (node-left t) d)))]
    [(> d (first (scratch  (node-key t)))) (rebalance (subst-R t (deleteavl (node-right t) d)))]
...
(define (listavl-helper l t)
  (cond
    [(empty? t) l]
    [else (listavl-helper 
           (cons (first (scratch  (node-key t))) (listavl-helper l (node-right t)))
           (node-left t))]))
...
(define (playgame n)
  (playgamehelp n empty)
  )

(define (playgamehelp n avl)
  (cond [(= (sizeavl avl) n) (listavl avl)]
      [else (playgamehelp n (insertavl avl (drawcard n)))]))
